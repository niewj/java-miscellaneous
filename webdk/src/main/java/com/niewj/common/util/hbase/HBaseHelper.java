...
import com.niewj.common.util.DateUtil;
import com.niewj.common.util.JSONUtil;
import com.niewj.common.util.SlowLogUtils;
...
public class HBaseHelper {
    ...
    public static void exec(String tableName, HBaseExec hbExec) {
        long start = System.currentTimeMillis();
        HTableInterface hTable = null;
        try {
            hTable = getHTable(tableName);
            ...
            throw new RuntimeException("exec error, tableName:" + tableName + ",exec:" + hbExec.logInfo(), e);
        } finally {
            closeHTable(hTable);
            SlowLogUtils.logHBaseResTime(tableName, "exec:" + hbExec.logInfo(), start);
        }
    }
    ...
    public static List<KeyValue> getValues(String tableName, String rowKey, String family, String startTime, String endTime) {
        ...
        try {
            ...
            Get get = new Get(rowKey.getBytes());
            get.addFamily(Bytes.toBytes(family));
            get.setTimeRange(DateUtil.convertStr2Date(startTime).getTime(), DateUtil.convertStr2Date(endTime).getTime());
            result = hTable.get(get);
            KeyValue[] kvs = result.raw();
            if (kvs.length > 0) {
                keyValues = new ArrayList<KeyValue>();
    ...
    public static List<Result> getResultsByRowkeyRangeAndColumnNames(String tableName, String rowkeyStart, String rowkeyEnd, List<String> columnNames,
        ...
        try {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("rowkeyStart:{},rowkeyEnd:{},columnNames:{},columnArray:{}", rowkeyStart, rowkeyEnd, JSONUtil.safeToJson(columnNames), JSONUtil.safeToJson(columnArray));
            }
    ...
    private static void closeHTable(HTableInterface hTable) {
        if (hTable != null) {
            try {
                ...
                LOG.warn("close hTable error, tableName:" + hTable.getName(), e);
            }
        }
    }
    ...
    public static void main(String[] args) throws UnsupportedEncodingException {
        ...
        data.put("bankCard", "62555559999967222");
